...
/*
void Renderer::renderSphere(GameObject *goSphere)
{
	Sphere* sphere = goSphere->getSphere();
	GLuint* textureID = sphere->getTextureID();
	vec3 v3Sphere;// = goSphere->getTransform()->getPosition();
	GLUquadric* quad;// = sphere->getQuad();
	GLdouble radius;// = sphere->getRadius();
	GLint slices;// = sphere->getSlices();
	GLint stacks;// = sphere->getStacks();
	...
	//glTranslatef(v3Sphere.x, v3Sphere.y, v3Sphere.z);
